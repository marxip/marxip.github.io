...
let currentPages = {
    root: 1,
    Ensayos_de_calibración: 1,
    errorLog: 1
};

// Función para renderizar posts filtrados por categoría
function renderPosts(category = 'root') {
    ...
    let filteredPosts = category === 'root' ? posts : posts.filter(post => post.category === category);
...
function openTab(event, tabName) {
    ...
    currentPages[tabName] = 1; // Reinicia solo la categoría actual
    renderPosts(tabName);
}
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    document.querySelectorAll(".tablinks").forEach(button => {
        button.addEventListener("click", event => {
            const tabName = button.dataset.tab || button.textContent.replace(/\s+/g, '_');
